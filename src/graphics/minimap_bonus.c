...
/*   Updated: 2025/08/23 15:22:37 by yannis           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_data_pixel *data_pixel)
{
	...
	while (1)
	{
		...
		if (data->map_data.map[data->ray_data.map_y][data->ray_data.map_x] == '1')
			break ;
		my_mlx_pixel_put(data_pixel, (int)data->ray_data.ray_x,
			(int)data->ray_data.ray_y, 0xFF0000);
...
void	display_minimap(t_data_game *data_game, t_data_pixel *data_pixel)
{
		...
		while (data_game->map_data.map[i][j])
		{
			...
			if (data_game->map_data.map[i][j] == '1')
			{
				while (y < 16)
				{
					...
					while (x < 16)
					{
						my_mlx_pixel_put(data_pixel, j * 16 + x, i * 16 + y,
							0xFFFFFF);
						x++;
					}
					y++;
	...
	while (y < 16)
	{
		...
		while (x < 16)
		{
			my_mlx_pixel_put(data_pixel,
				(int)(data_game->player_pos.player_pos_x * 16) + x,
				(int)(data_game->player_pos.player_pos_y * 16) + y, 0x000066);
			x++;
		}
		y++;
	}
